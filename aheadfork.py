...
def get_forks():
    ...
    # Should carry this header for all requests
    session.headers.update({'Authorization': 'token ' + GITHUB_API_TOKEN})
    ...
    if len(branches) == 1:
        # There is only one branch, so use it instead of looking for something non-existent.
    ...
    # Get forks
    # 30 items per page
    page_count = ceil(forks_count / 30)  
...
if __name__ == "__main__":
    ...
    try:
        forks = get_forks()
        if forks:
            # Only show forks that are ahead of the original repo. Sorted by ahead_by and pushed_at
